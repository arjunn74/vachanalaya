...
import { getChapter, getGitaNeighbors } from "@/data/gita";
import {
  FONT_SIZE_CLASS,
  LINE_HEIGHT_CLASS,
  useReaderPrefs,
...
export const Route = createFileRoute("/gita/$slug")({
  ...
  head: ({ loaderData }) => {
    ...
    const title = c ? `Bhagavad Gītā — Chapter ${c.number}: ${c.title} | Vachanalaya` : "Bhagavad Gītā — Vachanalaya";
    const desc = c
      ? `Chapter ${c.number} of the Bhagavad Gītā — ${c.title}. Sanskrit, transliteration and English translation.`
      : "Read the Bhagavad Gītā on Vachanalaya.";
    return {
      meta: [
        { title },
  ...
  component: GitaReader,
  notFoundComponent: () => (
    <div className="mx-auto max-w-xl px-5 py-24 text-center">
      <h1 className="font-serif text-3xl">Not found</h1>
      <p className="mt-2 text-muted-foreground">That chapter isn't in the library.</p>
      <Link to="/gita" className="mt-6 inline-block text-primary underline-offset-4 hover:underline">
        Back to the Gītā
      </Link>
    </div>
  ),
...
function GitaReader() {
  ...
  useEffect(() => {
    const key = `vachanalaya:progress:gita:${chapter.id}`;
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      if (e.key === "ArrowLeft" && prev) navigate({ to: "/gita/$slug", params: { slug: prev.id } });
      else if (e.key === "ArrowRight" && next) navigate({ to: "/gita/$slug", params: { slug: next.id } });
    ...
    <div className="min-h-screen bg-[var(--color-reader-bg)] text-[var(--color-reader-fg)] transition-colors">
      ...
      <header className="sticky top-0 z-30 border-b border-[var(--color-reader-rule)] bg-[var(--color-reader-bg)]/85 backdrop-blur">
        <div className="mx-auto flex h-14 max-w-3xl items-center justify-between gap-3 px-5">
          ...
          <div className="min-w-0 flex-1 text-center">
            <div className="truncate font-serif text-sm">{chapter.title}</div>
            <div className="text-[10px] uppercase tracking-[0.18em] text-[var(--color-reader-muted)]">
              Bhagavad Gītā · Chapter {chapter.number}
            </div>
          </div>
          <button
            type="button"
      ...
      >
        <header className="mb-12 border-b border-[var(--color-reader-rule)] pb-8">
          ...
          <h1 className="mt-3 font-serif text-4xl leading-tight tracking-tight md:text-5xl">{chapter.title}</h1>
        </header>
          ...
          {prev ? (
            ...
            >
              ...
              <div>
                ...
                <div className="mt-0.5 font-serif">Ch. {prev.number} · {prev.title}</div>
              </div>
            </Link>
          ) : (
            <span />
          ...
          {next ? (
            ...
            >
              <div>
                ...
                <div className="mt-0.5 font-serif">Ch. {next.number} · {next.title}</div>
              </div>
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div>
      <div className="mb-1.5 text-[10px] uppercase tracking-[0.18em] text-[var(--color-reader-muted)]">{label}</div>
      {children}
    </div>
  );
}
